...
class RpcServer extends Server
{
    ...
    public function run()
    {
        ...
        while (\true) {
            try {
                ...
                $server_reader = new ServerCallReader($event->call, $method_desc->request_type);
                try {
                    $this->processCall($method_desc, $server_reader, $server_writer, $context);
                } catch (\Exception $e) {
                    $context->setStatus(Status::status(STATUS_INTERNAL, $e->getMessage()));
                    $server_writer->finish();
                }
            } catch (\Exception $e) {
    ...
    private function processCall(MethodDescriptor $method_desc, ServerCallReader $server_reader, ServerCallWriter $server_writer, ServerContext $context)
    {

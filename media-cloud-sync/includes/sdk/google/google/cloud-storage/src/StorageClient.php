...
class StorageClient
{
    ...
    public function bucket($name, $userProject = \false, array $options = [])
    {
        ...
        return new Bucket($this->connection, $name, $options + ['requesterProjectId' => $userProject]);
    }
    /**
    ...
    public function buckets(array $options = [])
    {
        ...
        $bucketUserProject = $this->pluck('bucketUserProject', $options, null) ?? \true;
        ...
        return new BucketIterator(new PageIterator(function (array $bucket) use($userProject) {
            return new Bucket($this->connection, $bucket['name'], $bucket + ['requesterProjectId' => $userProject]);
        }, $callDelegate, $options + ['project' => $this->projectId], ['resultLimit' => $resultLimit]), $unreachable);
    }
    /**
    ...
    {
        $res = $this->connection->restoreBucket(['bucket' => $name, 'generation' => $generation] + $options);

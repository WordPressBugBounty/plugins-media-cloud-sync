 ...
 */
namespace Dudlewebs\WPMCS\Google\Cloud\Core\Testing\Snippet\Parser;

use Dudlewebs\WPMCS\Google\Cloud\Core\Testing\Reflection\ReflectionHandlerFactory;
use DOMDocument;
use Dudlewebs\WPMCS\Parsedown;
use ReflectionClass;
...
class Parser
{
    ...
    public function examplesFromClass($class)
    {
        ...
        if ($doc->getTags()) {
            ...
            foreach ($methods as $method) {
                $magicExamples = $this->examples($method['doc'], $class->getName() . '::' . $method['name'], $class->getFileName(), $class->getStartLine());
    ...
    public function examples(DocBlock $docBlock, $fullyQualifiedName, $file, $line, array $magicMethods = [])
    {
        ...
        foreach ($examples as $example) {
            ...
            $snippet = new Snippet($identifier, ['content' => $example->textContent, 'fqn' => $fullyQualifiedName, 'index' => $indexOrName, 'name' => $name, 'file' => $file, 'line' => $line]);

...
use Dudlewebs\WPMCS\Google\Cloud\Core\Testing\CheckForClassTrait;
use Dudlewebs\WPMCS\Google\Cloud\Core\Testing\Snippet\Container;
...
class SnippetTestCase extends TestCase
{
    ...
    public static function snippetFromMarkdown(string $fileName, string $header = '', int $index = 0)
    {
        ...
        foreach ($matches as $i => $match) {
            ...
            $language = isset($match[3]) ? \trim($match[3]) : '';
            ...
            $code = $match[4];
            $snippets[] = new Snippet($fileName . '-' . $i, ['content' => $code, 'file' => $fileName, 'index' => $i, 'name' => \strtolower($language)]);
        }

...
class CloudRunJobMetadataProvider implements MetadataProviderInterface
{
    ...
    public function __construct(array $env, ?Metadata $metadata = null)
    {
        ...
        $this->traceId = isset($env['HTTP_X_CLOUD_TRACE_CONTEXT']) ? \substr($env['HTTP_X_CLOUD_TRACE_CONTEXT'], 0, 32) : null;
    ...
    {
        return ['type' => 'cloud_run_job', 'labels' => ['job_name' => $this->serviceId, 'location' => $this->region, 'project_id' => $this->projectId()]];
    }
    /**
    ...
    {
        $labels = ['instanceId' => $this->instanceId, 'run.googleapis.com/execution_name' => $this->revisionId, 'run.googleapis.com/task_attempt' => $this->taskAttempt, 'run.googleapis.com/task_index' => $this->taskIndex, 'run.googleapis.com/trace_id' => $this->traceId];

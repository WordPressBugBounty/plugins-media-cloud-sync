    ...
    public function validateOptions(array $options, array|Message|string ...$optionTypes) : array
    {
        ...
        foreach ($optionTypes as $optionType) {
            if (\is_array($optionType)) {
                ...
                $messageKeys = \array_map(fn($method) => \lcfirst(\substr($method, 3)), \array_filter(\get_class_methods($optionType), fn($m) => 0 === \strpos($m, 'get')));
        ...
        if (!empty($options)) {
            throw new LogicException('Unexpected option(s) provided: ' . \implode(', ', \array_keys($options)));
        }
        return $splitOptions;
    }
}

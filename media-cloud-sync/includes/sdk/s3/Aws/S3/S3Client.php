...
use Dudlewebs\WPMCS\s3\Aws\Identity\S3\S3ExpressIdentityProvider;
...
use Dudlewebs\WPMCS\s3\Aws\S3\Parser\GetBucketLocationResultMutator;
use Dudlewebs\WPMCS\s3\Aws\S3\Parser\S3Parser;
use Dudlewebs\WPMCS\s3\Aws\S3\Parser\ValidateResponseChecksumResultMutator;
...
class S3Client extends AwsClient implements S3ClientInterface
{
    ...
    private function addBuiltIns($args)
    {
        ...
        if (\is_string($result)) {
            ...
            if ($result->isFallback() || $result->getEndpointsType() === 'legacy') {
                $value = \true;
            } else {
                $value = \false;
            }
